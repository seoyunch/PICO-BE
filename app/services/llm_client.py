...
class LLMClient:
    ...
    def _pestel_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            f"{_SECTION_FORMAT_RULE}"
            "1. Political (정치)\n"
            "(관련 정책·규제 동향을 2~3문장으로)\n\n"
            "2. Economic (경제)\n"
            "(관련 경제 상황·소비 여력을 2~3문장으로)\n\n"
            "3. Social (사회)\n"
            "(관련 사회적 인식·라이프스타일 변화를 2~3문장으로)\n\n"
            "4. Technological (기술)\n"
            "(활용 가능한 기술 트렌드를 2~3문장으로)\n\n"
            "5. Environmental (환경)\n"
            "(환경적 요인·지속가능성 이슈를 2~3문장으로)\n\n"
            "6. Legal (법률)\n"
            "(관련 법률·인증·컴플라이언스 이슈를 2~3문장으로)\n\n"
            "7. 종합 시사점\n"
            "(바로 앞 6번 Legal 항목과 절대 한 문단으로 이어 쓰지 말고, 반드시 이 제목을 "
            "단독 줄로 새로 시작한 다음 줄부터 서술해줘. 위 6가지 요인 중 이 아이디어에 "
            "특히 중요한 리스크·기회를 2~3개 짚어줘)"
    ...
    def _lean_canvas_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            "솔직히 밝혀줘)\n\n"
            "10. 핵심 가설 3개 및 검증 계획\n"
            "(위 9블록 중 이 사업의 성패를 가장 크게 좌우할 가설 3개를 뽑아서, 아래처럼 "
            "가설마다 대시(-)로 시작하는 항목 하나씩으로 나눠 써줘. 절대 문장으로 이어 "
            "쓰지 마.\n"
            "- 가설 1: (가설 내용) / 검증 계획: (어떻게 검증할지 - 예: 랜딩페이지 테스트, "
            "사전예약, 인터뷰)\n"
            "- 가설 2: (가설 내용) / 검증 계획: (...)\n"
            "- 가설 3: (가설 내용) / 검증 계획: (...))"
        )
    ...
    def _vpc_features_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            "| ... | ... | 상/중/하 | 상/중/하 |\n\n"
            "5. Use Case (3종)\n"
            "(위 핵심 기능이 실제로 어떻게 쓰이는지 구체적 시나리오 3개를, 아래처럼 "
            "대시(-)로 시작하는 항목 하나씩으로 나눠 써줘. 절대 문장으로 이어 쓰지 마.\n"
            "- Case 1: (시나리오, 2~3문장)\n"
            "- Case 2: (시나리오, 2~3문장)\n"
            "- Case 3: (시나리오, 2~3문장))"
        )
    ...
    def _mvp_roadmap_prompt(self, context: dict) -> str:
        ...
        return (
            ...
            f"{_SECTION_FORMAT_RULE}"
            "1. MoSCoW 분류\n"
            "(핵심 기능들을 아래처럼 대시(-)로 시작하는 4개 항목으로 나눠 써줘. 절대 "
            "문장으로 이어 쓰지 마. Won't have는 이번 범위에서 제외하는 이유도 함께.\n"
            "- Must have: (기능들)\n"
            "- Should have: (기능들)\n"
            "- Could have: (기능들)\n"
            "- Won't have: (기능들) / 제외 이유: (...))\n\n"
            "2. Kano 모델 매핑\n"
            "(Must have로 분류된 기능들을 아래처럼 기능 하나당 대시(-) 항목 하나씩으로 "
            "나눠서, 각각 Basic(당연)/Performance(성능)/Excitement(매력) 중 하나로 "
            "매핑해줘. 절대 문장으로 이어 쓰지 마.\n"
            "- (기능명) (Basic/Performance/Excitement): (이유)\n"
            "- (기능명) (Basic/Performance/Excitement): (이유))\n\n"
            "3. MVP 정의\n"
            "(아래처럼 대시(-)로 시작하는 3개 항목으로 나눠 써줘. 절대 문장으로 이어 "
            "쓰지 마.\n"
            "- Must have 전체: (...)\n"
            "- Performance 핵심: (...)\n"
            "- Excitement 1~2개: (...)\n"
            "그 다음 빈 줄을 하나 두고, 왜 이 범위로 MVP를 확정했는지 한 문단으로 "
            "설명해줘.)\n\n"
            "4. 마일스톤 & KPI\n"
            "(MVP 이후 2~3개월 단위 마일스톤 3~4개와 출시 후 확인할 KPI 2~3개를 아래처럼 "
            "대시(-)로 시작하는 항목 하나씩으로 나눠 써줘. 절대 문장으로 이어 쓰지 마.\n"
            "- 마일스톤 (몇 개월 내): (...)\n"
            "- KPI: (...))\n\n"
            "5. Epic 예시\n"
            "(MVP 기능을 Epic 2~3개로 묶어서, 아래 형식을 정확히 지켜서 하나씩 나열해줘. "
            "전부 한국어 문장으로만 작성해줘 - 영어 문장은 한 줄도 쓰지 마(User Story나 "
            "Given/When/Then 같은 영어 용어 자체를 절대 쓰지 마). Epic 제목이 있는 줄만 "
            "대시(-)로 시작하고, 그 아래 줄들은 들여쓰기만 하고 대시를 붙이지 마(그래야 한 "
            "Epic 안에 묶여서 보여):\n"
            "- Epic 1: (제목)\n"
            "  사용자 스토리: (역할)로서 나는 (하고 싶은 것)을 원한다, 왜냐하면 (그 이유) "
            "때문이다\n"
            "  인수 조건: (조건)일 때 (행동)하면 (결과)가 되어야 한다\n"
            "- Epic 2: (제목)\n"
            "  사용자 스토리: (한국어 문장으로)\n"
            "  인수 조건: (한국어 문장으로))"
        )
