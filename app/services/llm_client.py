...
class LLMClient:
    ...
    async def synthesize_draft(
        ...
        return await self._generate(
            ...
            f"{_SECTION_FORMAT_RULE}"
            "- 단, 3번 PESTEL 분석과 4번 Lean Canvas 섹션 안의 소제목 번호(1. 2. 3. ...)는 "
            "이 규칙의 예외야 - 그 번호와 소제목 문구는 아래 지시대로 반드시 그대로 남겨줘.\n\n"
            "# 1. 서비스 개요\n"
            "(분석 내용에서 유추할 수 있는 서비스 컨셉과 핵심 가치를 3~4문장으로)\n\n"
            "# 2. 시장조사 요약\n"
            "(시장조사 분석 내용을 핵심만 재구성)\n\n"
            "# 3. PESTEL 분석\n"
            "(아래 7개 소제목을 반드시 그대로 유지하면서, 각 제목 다음 줄에 1~2문장 요약을 "
            "적어줘 - 제목 문구 자체를 바꾸거나 하나로 합치지 마)\n"
            "1. Political (정치)\n"
            "2. Economic (경제)\n"
            "3. Social (사회)\n"
            "4. Technological (기술)\n"
            ...
            "7. 종합 시사점\n\n"
            "# 4. Lean Canvas\n"
            "(아래 10개 소제목을 반드시 그대로 유지하면서, 각 제목 다음 줄에 1~2문장 요약을 "
            "적어줘 - 제목 문구 자체를 바꾸거나 하나로 합치지 마. 10번째는 핵심 가설 3개를 "
            "목록으로)\n"
            "1. Problem\n"
            "2. Customer Segments\n"
            "3. Unique Value Proposition\n"
